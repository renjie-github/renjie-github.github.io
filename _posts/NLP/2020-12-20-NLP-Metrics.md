...
## 1. BLEU  
&emsp;&emsp;BLEU (bilingual evaluation understudy)用于评估**从一种语言翻译成另一种语言的文本的质量**。这里“质量”的好坏被定义为与人类翻译结果的一致性高低。BLEU分数的计算可以面向独立的翻译片段（一般是句子），也可以面向整个语料。语料级BLEU不是简单平均句子BLEU，而是先聚合n-gram计数，再计算修正precision、几何平均和 brevity penalty。**该度量方式不直接评价可理解性及语法的正确性**。BLEU的值介于0和1之间，越接近于1代表翻译结果越接近于“参照”。如果值为1，代表翻译“参照”中有一个与翻译结果相同（这也意味着，更多的“参照”将带来更高的分数）。  
...
## 2. METEOR  
&emsp;&emsp;METEOR（**M**etric for **E**valuation of **T**ranslation with **E**xplicit **OR**dering）是用来评估机器翻译效果的度量指标。**该指标基于precision和recall的调和平均，且recall的权重高于precision**。该指标被设计来修正BLEU的某些缺陷，并且在句子或语段层面上与人类判断产生良好的相关性。该指标的**主要思想是有时翻译模型意思正确，但它与参考翻译不一样（比如使用了同义词）。所以可以使用WordNet等知识源来扩展同义词集。此外，还考虑了单词的形式（有着相同词干的单词被认为是部分匹配的，也应该给予奖励，例如将likes翻译成like比翻译成其它的词要好）**。METEOR与BLEU度量的不同之处在于，BLEU更常在语料库层面寻求相关性。  
...
## 3. ROUGE
&emsp;&emsp;ROUGE（Recall-Oriented Understudy for Gisting Evaluation）是一组度量，用于比较自动生成的摘要或翻译与人类生成的参考摘要或翻译之间的相似性。**ROUGE与BLEU的区别是ROUGE更偏向召回率**，即更关注候选文本中覆盖了多少参考文本中的n-gram或序列结构。共包含如下5种评估度量：  
...
$$g_k(s_{ij})=\frac{h_k(s_{ij})}{\sum_{w_l\in\Omega} h_l(s_{ij})}\log\left(\frac{|I|}{\sum_{I_p\in I}\min(1,\sum_qh_k(s_{pq}))}\right) \tag{4.3}$$  
