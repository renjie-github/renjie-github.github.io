...
$$Q_{\text{target}}(s,a)\leftarrow r+\gamma \max_{a'\in A}Q(s',a')\tag{1}$$

&emsp;&emsp;Bellman更新公式中$\max$算子使用同一个Q值来同时做动作选择及动作评估，这使得它更容易选择高估的值，从而导致过于乐观的值估计。Double DQN的核心思想：将动作选择与评估解耦可以减小上述效应的影响。  
...
![Double DQN overestimation bound](/img/RL/DQN-variant-theorem.jpg)

&emsp;&emsp;与经典DQN的区别是：使用online network来评估贪心策略（进行动作选择），使用目标网络来估计值（进行值估计）。所以，Double DQN的目标Q值计算公式变为：  

$$Q_{\text{target}}(s,a)\leftarrow r+\gamma Q(s', \arg\max_{a'}Q(s',a';\theta_t);\theta_t^-)\tag{2}$$

&emsp;&emsp;上式中$\theta_{t}^{-}$是target网络的参数，$\theta_{t}$是当前online网络的参数。对应的 TensorFlow 代码实现为
```python
...
$$P(i)=\frac{p_i^\alpha}{\sum_kp_k^\alpha}\tag{3}$$

&emsp;&emsp;其中$p_i>0$是样本$i$的优先级。**指数$\alpha$表明了多大程度上使用了优先级，$\alpha=0$时退化为均匀采样，$\alpha=1$时为完全按照优先级采样**。  
&emsp;&emsp;优先级$p_i$的选择有两种实现方式：  
1. $p_i=|\delta_i|+\epsilon$，其中$\epsilon$是小的正常数来保证所有样本的采样概率都不为0。  
  &emsp;&emsp;为了实现从公式(3)对应的分布中高效地采样，线性时间复杂度$O(N)$无法满足需求。对于该实现方式，可以使用“sum-tree”的数据结构，支持对数时间复杂度的更新和采样。
2. $p_i=\frac{1}{\mathrm{rank}(i)}$，其中$\mathrm{rank}(i)$是replay buffer按照$|\delta_i|$排序（从高到低）后第$i$个样本的序号。  
...
&emsp;&emsp;上式中，如果$\beta=1$代表对非均匀采样进行完全补偿，$\beta=0$代表完全不进行补偿。这些权值被用在Q-learning的更新时：用$w_i\delta_i$代替$\delta_i$。出于稳定性的考虑，需要对权值进行归一化：除以$\max_i w_i$。  
&emsp;&emsp;在典型的强化学习场景中，由于策略，状态分布及bootstrap得到的目标的不断变化，训练过程是高度非平稳的。因此，在接近训练结束时，更新的准确性（无偏）更为关键。作者假设小的偏差可以被忽略（在训练早期，不平稳性因素较多）。为了使策略变得更灵活，作者引入了参数退火：训练过程中，参数$\beta$从初始的$\beta_0$逐渐增加至1（完全补偿）。实际操作中为线性变化。参数$\beta$与$\alpha$共同起作用，应同时增/减二者以调节参数（同时增加意味着采用更强的优先级采样，也因此需要更多的偏差矫正）。  
&emsp;&emsp;使用了Prioritized replay buffer的Double DQN算法如下图：  

![Prioritized replay buffer](/img/RL/prioritized_buffer.jpg)

&emsp;&emsp;使用 O(N) 时间复杂度 Prioritized replay buffer 的 Double DQN 算法 TensorFlow 实现。
#### **Dueling DQN**
...
![Dueling DQN architecture](/img/RL/dueling_model.jpg)

&emsp;&emsp;值得注意的是，虽然最终获得的是Q的估计值，但网络中作为状态值函数V以及优势值函数A的两部分网络所输出的值，不能被当作状态值V以及优势A的较好估计。  
> 这里的网络结构修改与预测“残差”的思想相近：显式拆分状态价值和动作优势，通常有助于训练过程更稳定。  

&emsp;&emsp;Dueling DQN网络结构所表达的含义可以写为$Q(s,a;\alpha,\beta)=V(s;\theta,\beta)+A(s,a;\theta,\alpha)$。但是虽然网络的两部分输出被设计用来估计状态和优势函数，但对于网络来说这一意图是“无法识别的”，为了解决这个问题，作者强制优势函数估计器对于所选动作优势值为0。具体实现是从优势输出估计中减去优势值中的最大值：

$$Q(s,a;\alpha,\beta)=V(s;\theta,\beta)+\left(A(s,a;\theta,\alpha)-\max_{a'\in\mathcal{A}}A(s,a';\theta,\alpha)\right)$$

&emsp;&emsp;这样便能一定程度上保证状态值估计器部分提供值函数的估计，优势值估计器部分提供关于优势的估计。  
&emsp;&emsp;另一种做法是减去均值：

$$Q(s,a;\alpha,\beta)=V(s;\theta,\beta)+\left(A(s,a;\theta,\alpha)-\frac{1}{|\mathcal{A}|}\sum_{a'}A(s,a';\theta,\alpha)\right)$$

&emsp;&emsp;这样做一方面使得V和A失去了明确的含义，因为对二者都引入了一个偏移。另一方面，这样做增加了优化的稳定性：优势估计只需要适应该部分均值的改变而不是需要适应最大值（最优动作的优势），而均值自然比最值要稳定很多。减去均值有助于提高“识别度”的同时并不会改变优势值（包括Q值）的相对顺序，从而保证了基于Q值的贪心算法的有效性。  
&emsp;&emsp;Dueling DQN 基于 TensorFlow 的算法实现。
#### **Noisy DQN**  
&emsp;&emsp;该内容出自2018年文章[NOISY NETWORKS FOR EXPLORATION](https://arxiv.org/abs/1706.10295)，该技术的提出是为了解决exploration vs. exploitation中的探索问题。大多数关于探索的强化学习研究都是依赖于对策略的随机扰动，比如$\epsilon$-greedy或者使用熵正则。另外还有添加“intrinsic reward”等方法，这些方法都需要算法使用者自定义参数，而不是从与环境的交互中学习。直接探索策略空间的方法（如进化或黑箱算法）通常要求与环境多次长时间交互。作者的核心观点是：对于权值向量的一个简单改变，可以在多个时间步上引发一致，复杂，状态依赖的策略改变。不同于添加无关扰动的方法（如$\epsilon$-greedy），这里的扰动是从一个噪声分布中采样得到的。扰动的方差是一个可学习的参数，该参数可以被看做注入的噪声的能量。  
&emsp;&emsp;噪声网络的权重与偏置都被参数化的噪声函数扰动，这些参数由梯度下降来更新。令$y=f_\theta(x)$代表由噪声参数$\theta$参数化的神经网络，将噪声参数$\theta$表示为$\theta\overset{\text{def}}{=}\mu+\Sigma\bigodot\varepsilon$，其中$\zeta\overset{\text{def}}{=}(\mu,\Sigma)$是一组可学习的参数向量，$\varepsilon$是一个具有**不变统计特性**的零均值噪声。  
...
- 因式分解高斯噪声  
  &emsp;&emsp;通过分解$\varepsilon_{i,j}^w$，可以使用$p$个标准正态分布变量$\varepsilon_i$作为输入噪声，$q$个标准正态分布变量$\varepsilon_i$作为输出噪声（所以总共$p+q$个高斯变量）。每个$\varepsilon_{i,j}^w$和$\varepsilon^b$可以被写作：  
	  $$\varepsilon_{i,j}^w=f(\varepsilon_i)f(\varepsilon_j)\quad\text{and}\quad\varepsilon_j^b=f(\varepsilon_j)\tag{6}$$

	  &emsp;&emsp;其中$f$是实值函数，作者使用的是$f(x)=\operatorname{sign}(x)\sqrt{|x|}$。对于噪声网络的损失函数$\overline{L}(\zeta)=\mathbb{E}[L(\theta)]$是噪声的期望，其梯度计算公式：  

	  $$\nabla\overline{L}(\zeta)=\nabla\mathbb{E}[L(\theta)]=\mathbb{E}[\nabla_{\mu,\Sigma}L(\mu+\Sigma\bigodot\varepsilon)]\tag{7}$$

	  &emsp;&emsp;作者对上述梯度使用蒙特卡洛近似，每一步选取一个样本$\xi$进行优化：  

	  $$\nabla\overline{L}(\zeta)\approx\nabla_{\mu,\Sigma}L(\mu+\Sigma\bigodot\xi)\tag{8}$$

&emsp;&emsp;Noisy DQN 基于 TensorFlow 的算法实现。
