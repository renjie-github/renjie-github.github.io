...
2. 通过与环境交互获得$(s,a,r,s')$，在这一步需要决定要采取何种动作，如何选择动作没有唯一的正确方法（exploration vs. exploitation）  
...
&emsp;&emsp;其中的$\delta_k(s,r,s')=r+\gamma \max_{a'\in A}Q(s',a')-Q(s,a)$就是所谓的TD error。  
#### **Deep Q Network**
...
&emsp;&emsp;DQN是**model-free**的：因为它通过直接使用从仿真器得到的样本来解决强化学习任务，而不需要显式地估计奖励与转移的动态特性$P(r,s'\mid s,a)$。DQN也是**off-policy**的：它在训练过程中学习的是贪心策略$a=\arg\max_{a'}Q(s,a';\theta)$，而其动作分布却保证了对状态空间一定程度的探索（**$\varepsilon$-greedy**），即以概率$\varepsilon$随机选择动作，以概率$1-\varepsilon$遵循贪心策略。  
...
&emsp;&emsp;作者还发现将误差项$r+\gamma \max_{a'}Q(s',a';\theta_i^-)-Q(s,a;\theta_i)$裁剪到(-1, 1)的范围内可提高算法稳定性。因为绝对值损失函数$\|x\|$对于有负值的$x$导数为-1，正值$x$导数为1。对误差项进行裁剪等效于：对超过(-1, 1)范围的误差使用绝对值损失函数。

> 这与深度学习中的 Huber loss 思路接近：减弱异常 TD error 对参数更新的影响，从而提高训练稳定性。  

&emsp;&emsp;完整的DQN算法如下：  

![DQN algorithm](/img/RL/DQN_algo.jpg)

&emsp;&emsp;该算法的 TensorFlow 实现中，关键更新部分代码如下：  
```python
...
def train_on_batch(self, states, actions, rewards, next_states, dones):
    ...
    # using one-hot operation to mask out irrelevant Q values
