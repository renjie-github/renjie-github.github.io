...
- Complete link（diameter）：两个cluster之间的相似度用两个cluster之间最不相似（最远）的样本之间的相似度。合并cluster是按照合并后具有最小diameter的原则进行。这一行为不是局部性的，可以获得更加“紧凑”的cluster。此外，该方法也对异常值敏感
...
- Group Average Agglomerative Clustering（GAAC）：假设cluster$C_a$和cluster$C_b$合并为cluster$C_{a\cup b}$，则新的centroid为：
  $$
...
- Ward's Criterion：使用合并之后SSE（Sum of Squares Error），其中SSE的计算是对每个cluster内所有样本计算的。当两个cluster合并后，SSE会增加，Ward's criterion方法使用的便是选择合并后SSE增加量最小的两个cluster进行合并：  
  $$
  W(C_{a\cup b}, \boldsymbol{c}_{a\cup b})-W(C, \boldsymbol{c})=\frac{N_aN_b}{N_a+N_b}d^2(\boldsymbol{c}_a, \boldsymbol{c}_b)
  $$
...
&emsp;&emsp;该方法决定划分哪个cluster的原则是：选择具有最大SSE的cluster进行划分。划分方式可以是使Ward's criterion减少最多的划分方式。对于categorical数据，可以使用Gini-index。该方法使用一个阈值来决定终止条件，不产生太小的cluster（因为这些cluster包含的主要是噪声）。
...
&emsp;&emsp;该方法的核心思想是**多级聚类（Multi-level clustering）**，包括low-level的micro-clustering：减少复杂度并增加可扩展性，以及high-level的macro-clustering：为更高层次的聚类留出足够的灵活性。BIRCH的时间复杂度是线性的，它通过扫描一遍数据来找到一个比较好的聚类，然后通过额外的几次扫描来改善聚类质量。  
